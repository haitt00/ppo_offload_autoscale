...
model.learn(total_timesteps=PPO_training_time)
...
set_seed(rand_seed)
obs = env.reset()
print("results for only 0")
for i in range(t_range):
    action = [0.0]
    obs, rewards, dones, info = env.step(action)
    rewards_list_only_0.append(1 / rewards)
    avg_rewards_only_0.append(np.mean(rewards_list_only_0[:]))
    print("t={}".format(i))
    env.render()
    print(action)
#ppo
print("results for ppo")
set_seed(rand_seed)
obs = env.reset()
for i in range(t_range):
...
print('--RESULTS--')
print("trainning time = {}".format(PPO_training_time))
print('{:15}{:30}'.format('method','total cost'))
print('{:15}{:<30}'.format('PPO', avg_rewards_ppo[-1]))
print('{:15}{:<30}'.format('only 0', avg_rewards_only_0[-1]))
